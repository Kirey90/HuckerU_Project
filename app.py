...
def clear_data(path):
	subprocess.call(['spark-submit', 'clear.py', path], shell=True)
...
class DB:
	...
	def updateFlatTable(self):
		...
		self.cursor.execute('''
			...
			floor, m2_room, region, city_type, update_date) 
			select flat_id, rooms, m2, price, price_m, city, 
			floor, m2_room, region, city_type, update_date from flat_02;
		''')
		...
		self.cursor.execute('''
			...
			floor, m2_room, region, city_type, update_date) 
			select flat_id, rooms, m2, price, price_m, city, 
			floor, m2_room, region, city_type, update_date from flat_01;
		''')
